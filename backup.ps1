...
Get-ChildItem -Path $targetDir -Recurse -Force | Where-Object { !$_.PSIsContainer -and $_.CreationTime -lt $pastLimit } | Remove-Item -Force -Verbose | Out-File -Encoding ascii $logFileName

LogWrite -logstring 'borrando archivos *.sql'
Remove-Item -Path $(Join-Path -Path $alternateDir -ChildPath *.sql) -Verbose | Out-File -Encoding ascii $logFileName
Remove-Item -Path $(Join-Path -Path $targetDir -ChildPath *.sql) -Verbose | Out-File -Encoding ascii $logFileName 
